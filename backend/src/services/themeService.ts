...
export class ThemeService {
	...
	async getUserTheme(userId: string): Promise<UserTheme> {
		try {
			...
			if (userBadges.length === 0) {
				console.log(
					`[DEBUG] No badge for the user ${userId}, return to default theme`,
				);
				return "default";
			}
			...
			for (const userBadge of userBadges) {
				if (userBadge.badge?.themeId) {
					try {
						const theme = await this.getThemeById(
							userBadge.badge.themeId.toString(),
						);
						if (theme && theme.isActive) {
							associatedThemes.push(theme);
						} else if (theme && !theme.isActive) {
							console.log(
								`Theme ${theme.name} (${theme.slug}) disabled for badge ${userBadge.badge.name}, switch to default theme`,
							);
						}
					} catch (err) {
						console.warn(
							`Theme ${userBadge.badge.themeId} not found for badge ${userBadge.badge.name}`,
						);
					}
				}
			}
	...
	async getUserThemeInfo(userId: string) {
		...
		if (!predefinedThemes.includes(themeIdentifier)) {
			try {
				const customTheme = await Theme.findOne({
		...
		return {
			theme: themeIdentifier,
			...(themeInfos[themeIdentifier as keyof typeof themeInfos] ||
				themeInfos.default),
		};
	}
