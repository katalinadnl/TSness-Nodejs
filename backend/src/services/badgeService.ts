...
import { UserBadge, type IUserBadge } from "../models/UserBadge";
...
export class BadgeService {
	...
	async getAllBadgesWithStatus(
		userId: string,
	): Promise<{ badge: IBadge; earned: boolean; earnedAt?: Date }[]> {
		try {
	...
	async getUsersLeaderboard(): Promise<
		{ user: any; badgeCount: number; badges: any[] }[]
	> {
		try {
			...
			for (const user of users) {
				...
				console.log(
					`DEBUG - User ${user.username} has ${userBadges.length} badges`,
				);
