...
import {
	ChallengeAndParticipationStatus,
	UserRole,
...
export class ChallengeService {
	...
	async createChallenge(
		...
		if (creatorRole === UserRole.GYM_OWNER) {
			...
			if (data.equipment?.length > 0) {
				...
				const allEquipmentsValid = data.equipment.every((eq: string) =>
					allEquipments.includes(eq),
				);
				if (!allEquipmentsValid) {
					throw new Error(
						"Some equipment is not linked to the rooms of your gym.",
					);
				}
			}
			...
			if (data.recommendedExerciseTypeIds?.length > 0) {
				...
				const allExerciseValid = data.recommendedExerciseTypeIds.every(
					(id: string) => validExerciseTypeIds.includes(id),
				);

				if (!allExerciseValid) {
					throw new Error(
						"Some recommended exercise types are not linked to the rooms of your gym.",
					);
				}
			}
		}
	...
	async getAllChallenges(filters: any = {}): Promise<any[]> {
		...
		if (filters.minDuration || filters.maxDuration) {
			...
			if (filters.minDuration)
				query.duration.$gte = Number(filters.minDuration);
			if (filters.maxDuration)
				query.duration.$lte = Number(filters.maxDuration);
		}
	...
	async participateInChallenge(challengeId: string, userId: string) {
		...
		const already = challenge.participants.some((p) =>
