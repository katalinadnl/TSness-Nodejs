...
export interface ExerciseTypeDocument
	extends Omit<ExerciseType, "id">,
		Document {}
...
const exerciseTypeSchema = new Schema<ExerciseTypeDocument>(
	{
		...
		targetedMuscles: {
			...
			validate: {
				validator: (muscles: string[]) => muscles && muscles.length > 0,
...
export const ExerciseType = mongoose.model<ExerciseTypeDocument>(
	"ExerciseType",
	exerciseTypeSchema,
);
