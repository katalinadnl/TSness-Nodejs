...
class ChallengeSocialController {
	inviteToChallenge: RequestHandler = async (
		req: Request,
		res: Response,
		next: NextFunction,
	) => {
		try {
			...
			if (challenge.participants.some((p) => p.userId.equals(userId))) {
				res
					.status(400)
					.json({ message: "Utilisateur déjà invité ou participant." });
				return;
			}
	...
	acceptChallengeInvitation: RequestHandler = async (
		req: Request,
		res: Response,
		next: NextFunction,
	) => {
		try {
			...
			if (participant.status !== ChallengeAndParticipationStatus.INVITED) {
				res
					.status(400)
					.json({ message: "Invitation déjà acceptée ou terminée." });
				return;
			}
	...
	refuseChallengeInvitation: RequestHandler = async (
		req: Request,
		res: Response,
		next: NextFunction,
	) => {
	...
	getUserInvitations: RequestHandler = async (
		req: Request,
		res: Response,
		next: NextFunction,
	) => {
	...
	buildRoutes() {
		...
		router.post(
			"/:challengeId/accept",
			this.acceptChallengeInvitation.bind(this),
		);
		router.post(
			"/:challengeId/refuse",
			this.refuseChallengeInvitation.bind(this),
		);
