...
import {
	...
	type UpdateExerciseTypeRequest,
} from "../models/ExerciseType";
...
export class ExerciseTypeController {
	...
	async createExerciseType(req: Request, res: Response): Promise<void> {
		try {
			const { name, description, targetedMuscles }: CreateExerciseTypeRequest =
				req.body;
	...
	async updateExerciseType(req: Request, res: Response): Promise<void> {
		try {
			const { id } = req.params;
			const { name, description, targetedMuscles }: UpdateExerciseTypeRequest =
				req.body;
	...
	async deleteExerciseType(req: Request, res: Response): Promise<void> {
		try {
			...
			const associatedRooms = await TrainingRoom.find({
				assignedExerciseTypeId: id,
			});
			if (associatedRooms.length > 0) {
				res.status(400).json({
					success: false,
					message:
						"Cannot delete exercise type as it is assigned to training rooms",
					data: {
	...
	async getTrainingRoomsByExerciseType(
		req: Request,
		res: Response,
	): Promise<void> {
		try {
			...
			const trainingRooms = await TrainingRoom.find({
				assignedExerciseTypeId: id,
			})
	...
	buildRoutes() {
		...
		router.get(
			"/:id/training-rooms",
			this.getTrainingRoomsByExerciseType.bind(this),
		);
